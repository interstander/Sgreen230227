...
-- 1번  덧셈연산을 이용하여 모든 사원에 대하여 $300
--      의 급여 인산을 계산한 후 사원의 이름, 급여, 
--      인상된 급여를 출력하세요
SELECT ename, salary, salary+300 AS "인상된 급여" 
FROM employee;

--  2번   사원의 이름, 급여, 연간 총수입을 총수입이 많은 것 부터 작은 순으로 출력하세요. 
-- 연간 총수입은 월급에 *12를 한 후 $100의 상여금을 더해서 계산하세요.
SELECT ename, salary, salary*12+100 AS "연간총수입"
FROM employee
ORDER BY "연간총수입" DESC;

--  3번  급여가 2000이 넘는 사원의 이름과 급여를 급여가 많은 것부터 작은 순으로 출력하세요.
SELECT ename, salary    -- 궁극적으로 출력할 내용
FROM employee           -- 어디서 가져왔나
WHERE salary >= 2000    -- 조건
ORDER BY salary DESC;   -- 정렬

-- 4번  사원 번호가 7788인 사원의 이름과 부서번호를 출력하세요
SELECT ename, dno
FROM employee
WHERE eno=7788;

--5번 급여가 2000에서 3000사이에 포함되지 않는 사원의 이름과 급여를 출력하세요
SELECT ename, salary
FROM employee
...
--9. 1981년도에 입사한 사원의 이름과 입사일을 출력하세요(LIke연산자와 와일드카드 사용)
...
--13. 이름에 A와 E를 모두 포함하는 사원이 이름을 출력하세요.
SELECT ename
FROM employee
